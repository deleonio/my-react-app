...
def main():
    ...
    # Analysiere und sortiere alle Tickets
    moved_count = {}
    total = 0

    print("Sortiere Tickets nach Synergie-Score...")
    print()
    ...
    for ticket_file in sorted(selected_dir.glob('issue-*.md'), key=lambda x: int(x.stem.split('-')[1])):
        ...
        target_dir = selected_dir / folder
        ...
        print(f"✓ {ticket_file.name:20} Score {score:3} → {folder}")
    ...
    print("=" * 80)
    print()
    print(f"Gesamt verschobene Tickets: {total}")
    print()
